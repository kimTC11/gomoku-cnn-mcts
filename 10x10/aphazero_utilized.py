...
class MCTS:  
    """  
    This class handles the MCTS tree with thread-safe operations for parallel simulations.  
    """  
    ...
    def __init__(self, game, nnet, args):  
        ...
        # Thread safety for parallel MCTS simulations
        self.lock = threading.Lock()
        self.use_threads = getattr(args, 'use_threads', True)  
  
    def getActionProb(self, canonicalBoard, temp=1):  
        """  
        This function performs numMCTSSims simulations of MCTS starting from  
        canonicalBoard using multithreading + GPU for maximum speed.  
  
        Returns:  
            probs: a policy vector where the probability of the ith action is  
                   proportional to Nsa[(s,a)]**(1./temp)  
        """  
        # Use multithreading for parallel MCTS simulations (threads share GPU safely)
        num_threads = getattr(self.args, 'num_threads', 8)
        
        if self.use_threads and self.args.numMCTSSims >= 100 and num_threads > 1:
            # Parallel execution with ThreadPoolExecutor
            with ThreadPoolExecutor(max_workers=num_threads) as executor:
                # Submit all simulations
                futures = [executor.submit(self.search, canonicalBoard) 
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        log.error(f"MCTS search error: {e}")
        else:
            # Sequential execution for small simulations or when threads disabled
            for _ in range(self.args.numMCTSSims):  
    ...
    def _single_search(self, canonicalBoard):  
        """  
        Helper function for parallel MCTS search.  
        Creates a temporary MCTS instance for independent search.  
        """  
        # Create a temporary MCTS instance for this search  
        temp_mcts = MCTS(self.game, self.nnet, self.args)  
        return temp_mcts.search(canonicalBoard)  
    ...
    def search(self, canonicalBoard):  
        ...
        # Thread-safe terminal check
        with self.lock:
            if s not in self.Es:  
                self.Es[s] = self.game.getGameEnded(canonicalBoard, 1)  
            terminal_value = self.Es[s]
        
        if terminal_value is not None:  
            # terminal node  
            return terminal_value  
  
        # Thread-safe leaf node check and expansion
        with self.lock:
            is_leaf = s not in self.Ps
        
        if is_leaf:  
            # leaf node - GPU prediction (threads can share GPU safely)
            policy, v = self.nnet.predict(canonicalBoard)  
            valids = self.game.getValidMoves(canonicalBoard, 1)  
            policy = policy * valids  # masking invalid moves  
            sum_Ps_s = np.sum(policy)  
            if sum_Ps_s > 0:  
...
def execute_episode_worker(args_tuple):  
    ...
    Each worker runs an independent episode.  
    """  
    game, nnet_class, nnet_args, episode_id = args_tuple  
      
    # Create new instances for this worker  
    nnet = nnet_class(game, nnet_args)  
    if nnet_args.load_model_file:  
        nnet.load_checkpoint(nnet_args.checkpoint, nnet_args.load_model_file)  
    ...
    while True:  
        ...
        temp = int(episodeStep < nnet_args.tempThreshold)  
        ...
        if r is not None:  
            # r * (1 if curPlayer == x[1] else -1) means 1 for winner, -1 for loser, 0 for draw.  
            return [  
                (x[0], x[2], r * (1 if curPlayer == x[1] else -1))  
...
class SelfPlay:  
    """  
    This class executes the self-play + learning with parallel execution.  
    """  
    ...
    def learn(self):  
        ...
        iteration. Uses multiprocessing for parallel self-play execution.  
        ...
        for i in range(1, self.args.numIters + 1):  
            ...
            # Use multiprocessing for self-play episodes  
            if hasattr(self.args, 'num_workers') and self.args.numEps > 10:  
                # Parallel execution  
                log.info(f"Running {self.args.numEps} episodes in parallel with {self.args.num_workers} workers")  
                  
                # Prepare arguments for worker processes  
                worker_args = []  
                for episode_id in range(self.args.numEps):  
                    # Create a copy of args for each worker  
                    worker_args_copy = dotdict(self.args)  
                    worker_args_copy.load_model_file = None  # Workers don't need to load model  
                ...
                with Pool(processes=self.args.num_workers) as pool:  
                    episode_results = pool.map(execute_episode_worker, worker_args)  
                  
                # Collect results  
                for examples in episode_results:  
...
def load_config(config_path):  
    ...
    args.num_workers = config['system'].get('num_workers', min(4, os.cpu_count() or 1))  
    args.num_threads = config['system'].get('num_threads', 8)  
...
def print_config(args):  
    ...
    print(f"  Training Workers (multiprocessing): {args.num_workers}")
    print(f"  Gameplay Threads (multithreading): {getattr(args, 'num_threads', 8)}")
    print("==================\n")  
def main():  
    import argparse  
    ...
    parser.add_argument("--num_workers", type=int, default=None, help="Number of worker processes for training multiprocessing")  
    parser.add_argument("--num_threads", type=int, default=20, help="Number of threads for MCTS simulations during gameplay")
    ...
    if args.play:  
        def getPlayFunc(name):  
            if name == "human":  
                ...
                # Use hybrid GPU + multithreaded MCTS for maximum speed
                num_threads = getattr(args, 'num_threads', 8)
                log.info(f"Using {num_threads} threads for parallel MCTS with GPU acceleration")
